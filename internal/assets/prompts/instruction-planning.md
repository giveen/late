...
* **CONTEXT EFFICIENCY**: Every tool call is persisted to the session history and consumes LLM context. Be strategic:
  ...
  * **CRITICAL — no redundant verification**: After `read_file` with `read_imports: true` on a directory, the output already lists every `.go` file, its line count, import count, and package name. Do NOT call `search_tool(search_names:true, pattern:"*.go")` to "verify" the file list — that adds tool calls without new information. Trust the `read_imports` output. If you already know what files are in a directory, don't search for them.
  * `search_tool` with `search_names: true` matches filenames by **glob** (e.g. `search_tool(pattern="*.go", search_names:true, path:"internal/foo")`) — not regex. Use globs like `*.go`, `*_test.go`, `*foo*`. (If you accidentally use regex like `\.go$`, it gets auto-converted.)
