...
- **Context Efficiency**: Every tool call is persisted. Use `read_file` with `read_imports: true` on a directory to get all imports in one call (preferred for exploration). Use `read_file` with `read_imports: true` on a file to check one file's imports. Use `search_tool` with `search_names: true` for filename globbing (globs like `*.go`, not regex). Avoid dumping `go.mod`/`go.sum`. **After `read_imports` on a directory, do NOT call `search_tool` to verify the file list — the import output already has every file.**
