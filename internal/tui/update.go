	...
	case tea.KeyMsg:
		...
		case "enter":
			...
			if strings.HasPrefix(input, "/") {
				return m.handleSlashCommand(input, focusedState)
			}
...
func (m Model) handleSlashCommand(input string, focusedState *AppState) (Model, tea.Cmd) {
	...
	case "/log":
		...
		return m, nil
	default:
		m.Err = fmt.Errorf("unknown command: %s. Available: /quit, /exit, /help, /clear, /log", cmd)
		return m, nil
	}
}
